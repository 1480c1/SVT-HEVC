	...
	EB_U32                 componentMask)
{
	...
	if (componentMask & PICTURE_BUFFER_DESC_LUMA_MASK) {
		...
		uniPredLumaIFFunctionPtrArrayNew[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 2)](
	...
	if (componentMask & PICTURE_BUFFER_DESC_CHROMA_MASK) {
		...
		uniPredChromaIFFunctionPtrArrayNew[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 3)](
    ...
    EB_S16                *tempBuf0)                //input parameter, please refer to the detailed explanation above.
{
	...
	uniPredLuma16bitIFFunctionPtrArray[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 2)](
	...
	EB_U32				   componentMask)
{
	...
	if (componentMask & PICTURE_BUFFER_DESC_LUMA_MASK) {
		...
		else
		{
			...
			biPredLumaIFFunctionPtrArrayNew[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 2)](
			...
			// bi-pred luma clipping
			biPredClippingFuncPtrArray[!!(ASM_TYPES & AVX2_MASK)](
				puWidth,
				puHeight,
				refList0TempDst,
				refList1TempDst,
	...
	if (componentMask & PICTURE_BUFFER_DESC_CHROMA_MASK) {
		...
		else
		{
			...
			biPredChromaIFFunctionPtrArrayNew[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 3)](
    ...
    EB_S16                *fistPassIFTempDst)
{
	...
	else
	{
		...
		biPredLumaIFFunctionPtrArrayNew16bit[!!(ASM_TYPES & AVX2_MASK)][fracPosx + (fracPosy << 2)](
			(EB_U16 *)fullPelBlockL0->bufferY + 4 + 4 * fullPelBlockL0->strideY,
			fullPelBlockL0->strideY,
			refList0TempDst,
			puWidth,
		...
		biPredClipping16bitFuncPtrArray[!!(ASM_TYPES & AVX2_MASK)](
			puWidth,
			puHeight,
			refList0TempDst,
			refList1TempDst,
