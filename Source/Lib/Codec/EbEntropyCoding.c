    ...
    TransformUnit_t              *tuPtr)
{
    ...
    if (transCoeffShape && isOnlyDc == EB_FALSE) {
        PicZeroOutCoef_funcPtrArray[(EB_ASM_C & AVX2_MASK) && 1][(size >> 1) >> 3](
            coeffBufferPtr,
            coeffStride,
            (size >> 1),
            (size >> 1),
        ...
        if (transCoeffShape == N4_SHAPE) {
            PicZeroOutCoef_funcPtrArray[(EB_ASM_C & AVX2_MASK) && 1][(size >> 2) >> 3](
                coeffBufferPtr,
                coeffStride,
                (size >> 2),
                (size >> 2),
	...
	EbPictureBufferDesc_t  *coeffPtr)
{
	...
	if (tuPtr->lumaCbf) {
		EncodeQuantizedCoefficientsFuncArray[!!(ASM_TYPES & AVX2_MASK)](
			cabacEncodeCtxPtr,
			tuSize,
	...
	CabacCost_t               *CabacCost)
{
	...
	if (yCountNonZeroCoeffs) {
        ...
        if(coeffCabacUpdate)
            EstimateQuantizedCoefficientsUpdate[!!(ASM_TYPES & AVX2_MASK)](
	...
	if (componentMask & PICTURE_BUFFER_DESC_Y_FLAG) {
		...
		if (yCountNonZeroCoeffs) {
			...
			if (coeffCabacUpdate)
			    ...
			    EstimateQuantizedCoefficients[encoderModeIndex][!!(ASM_TYPES & AVX2_MASK)](
				    CabacCost,
				    cabacEncodeCtxPtr,
				    (transformSize >> partialFrequencyN2Flag),
				    type,
